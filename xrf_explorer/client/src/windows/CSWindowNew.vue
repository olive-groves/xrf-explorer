...
import { Label } from "@/components/ui/label"; // Added Label import explicitly if not already auto-imported
import { Button } from "@/components/ui/button"; // Added Button import explicitly
import {
  NumberField,
  NumberFieldContent,
...
// Mode toggle state
type SegmentationMode = "complete" | "elements";
...
async function fetchColors() {
  ...
  try {
    ...
    currentError.value = "Network error occurred";
    status.value = Status.ERROR;
    return false;
  }
}
...
async function generateColors() {
  try {
    const success = await fetchColors();
    if (success) {
...
function getElementIndex(elementName: string | undefined) {
  if (elementName == undefined || elementName == "complete") {
    return 0;
  }
...
/**
 * Adds a new element item with default values into the list of elements.
 */
 ...
 * @returns SelectionAreaSelection covering the full image.
 */
...
<template>
  <Window title="Color segmentation" location="right" :disabled="!elementalDataPresent">
    <div class="space-y-2 p-2">
      ...
      <!-- CLUSTER NUMBER SELECTION (Common to both modes) -->
      ...
      <!-- ELEMENTS SELECTION (Only shown in Elements mode) -->
        ...
        <div class="flex items-center space-x-4" v-for="(elementSel, index) in elementsSelected" :key="elementSel.id">
          <div class="max-w-36 grow space-y-1">
            ...
            <Select v-model="elementSel.name" class="w-full">
              ...
              <SelectContent>
                <!-- 'Complete painting' removed from here -->
                <SelectItem
      ...
      <!-- ACTION BUTTON -->
      <Button class="mt-4 w-full" @click="generateColors">Generate color clusters</Button>
