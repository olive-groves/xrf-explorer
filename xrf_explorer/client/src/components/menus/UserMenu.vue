...
import { LoginDialog, LogoutDialog, CreateAccountDialog, ManageAccountsDialog, ManageUserDialog } from "@/components/workspace";
import { ref } from "vue";
...
<template>
  <Dialog v-model:open="dialogOpen" @update:open="reset">
    ...
    <ManageUserDialog v-if="window == 'manage-user'" :user="{original_username: originalName, username: userName, role: userRole}" @close="window='manage-accounts', originalName = '', userName = '', userRole = ''" />
  </Dialog>
</template>